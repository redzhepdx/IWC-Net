...
def cap_U_encoder(input, K):
    ...
    conv_caps_6    = SegConvolution(conv_caps_5, 5, 8, 32, strides=1, routings=3)
    print("Conv Caps 6 Shape : ", conv_caps_6.get_shape())

    deconv_caps_1  = SegConvolution(conv_caps_6, 4, 8, 32, strides=2, routings=3, op='deconv')
    print("Deconv 1 Shape : ", deconv_caps_1.get_shape())
    concat_1       = tf.concat([deconv_caps_1, conv_caps_4], axis=3)
    conv_caps_7    = SegConvolution(concat_1, 5, 4, 16, strides=1, routings=3)
    print("Conv Caps 7 Shape : ", conv_caps_7.get_shape())

    deconv_caps_2  = SegConvolution(conv_caps_7, 4, 4, 16, strides=2, routings=3, op='deconv')
    print("Deconv 2 Shape : ", deconv_caps_2.get_shape())
    concat_2       = tf.concat([deconv_caps_2, conv_caps_2], axis=3)
    conv_caps_8    = SegConvolution(concat_2, 5, 4, 16, strides=1, routings=3)
    print("Conv Caps 8 Shape : ", conv_caps_8.get_shape())

    deconv_caps_3  = SegConvolution(conv_caps_8, 4, 2, 16, strides=2, routings=3, op='deconv')
    print("Deconv 3 Shape : ", deconv_caps_3.get_shape())
    concat_3       = tf.concat([deconv_caps_3, conv1_expanded], axis=3)
    conv_caps_9    = SegConvolution(concat_3, 1, 1, K, strides=1, routings=3)
    print("Conv Caps 9 Shape : ", conv_caps_9.get_shape())

    squeeze        = tf.squeeze(conv_caps_9, axis=3)
...
def cap_U_decoder(input):
    ...
    dec_conv_caps_6    = SegConvolution(dec_conv_caps_5, 5, 8, 32, strides=1, routings=3)
    print("Conv Caps 6 Shape : ", dec_conv_caps_6.get_shape())

    dec_deconv_caps_1  = SegConvolution(dec_conv_caps_6, 4, 8, 32, strides=2, routings=3, op='deconv')
    print("Deconv 1 Shape : ", dec_deconv_caps_1.get_shape())
    dec_concat_1       = tf.concat([dec_deconv_caps_1, dec_conv_caps_4], axis=3)
    dec_conv_caps_7    = SegConvolution(dec_concat_1, 5, 4, 16, strides=1, routings=3)
    print("Conv Caps 7 Shape : ", dec_conv_caps_7.get_shape())

    dec_deconv_caps_2  = SegConvolution(dec_conv_caps_7, 4, 4, 16, strides=2, routings=3, op='deconv')
    print("Deconv 2 Shape : ", dec_deconv_caps_2.get_shape())
    dec_concat_2       = tf.concat([dec_deconv_caps_2, dec_conv_caps_2], axis=3)
    dec_conv_caps_8    = SegConvolution(dec_concat_2, 5, 4, 16, strides=1, routings=3)
    print("Conv Caps 8 Shape : ", dec_conv_caps_8.get_shape())

    dec_deconv_caps_3  = SegConvolution(dec_conv_caps_8, 4, 2, 16, strides=2, routings=3, op='deconv')
    print("Deconv 3 Shape : ", dec_deconv_caps_3.get_shape())
    dec_concat_3       = tf.concat([dec_deconv_caps_3, dec_conv1_expanded], axis=3)
    dec_conv_caps_9    = SegConvolution(dec_concat_3, 1, 1, 16, strides=1, routings=3)
    print("Conv Caps 9 Shape : ", dec_conv_caps_9.get_shape())

    dec_squeeze        = tf.squeeze(dec_conv_caps_9, axis=3)
